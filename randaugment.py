...
def level_to_arg(hparams,name,level):

    if name in ['AutoContrast','Equalize','Invert']:
        return ()
    if name == 'Rotate':
        return _rotate_level_to_arg(level)
    if name == 'Posterize':
...
def distort_image_with_randaugment(image, num_layers, magnitude,
                                cutout_const=40, translate_const=100,cutout_op = True):
    ...
    '''
    replace_value = [128] * 3
    augmentation_hparams = {"cutout_const":cutout_const,"translate_const":translate_const}
    available_ops = [
